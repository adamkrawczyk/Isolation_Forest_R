...
print_mat_data_info <- function(data) {
  arrhytmia_dim <- dim(data$X)
  print(paste("Rozmiar X: N=", dim(data$X)[1], ", M=", dim(data$X)[2], sep=""))
  print(paste("Rozmiar Y:", dim(data$y)[1]))
}
```
...
run_evaluation_metrics <- function(pred, expected) {
  ...
  print(conf)
  return(list(precision=precision, recall=recall, accuracy=conf$overall[1]))
}
...
LOF_algo = "LOF"
Rabom_Forest_algo = "Random Forest"

run_algorithm_and_print_result <- function(data_df, name_of_algo, split_factor=0.8) {
...
```{r}
test_satellite_data <- load_mat_data("data/temporary/satellite.mat")
test_satellite_data_df <- prepare_mat_data(test_satellite_data)
print_mat_data_info(test_satellite_data)
```

```{r}


run_algorithm_and_print_result(cover_data_df, KNN_algo)
