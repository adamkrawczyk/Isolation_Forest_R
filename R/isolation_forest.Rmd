...
run_isolation_forest <- function(split_df, sub_set_size=256, num_of_trees = 100) {
  iforest<- isolationForest$new(sample_size=sub_set_size, num_trees=num_of_trees)
  iforest$fit(split_df$train_no_label)
  pred <- iforest$predict(split_df$train_no_label)
...
#pred <- factor(run_isolation_forest(split_train_test_df))

#expected <-  factor(split_df$train_label)
...
pred <- factor(run_implemented_isolation_forest(split_train_test_df))
...
plot(perf, col="blue")
plot(perf2, add = TRUE, col="red", sub="Sub-title")
abline(0,1)
#pred 
```
