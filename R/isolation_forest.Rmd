...
run_if_with_param <- function(test_data_name, data_df, sub_set_size, num_of_tree) {
  ...
  TPR <- round(result_list$conf$byClass[[1]], 4)
  TNR <- round(result_list$conf$byClass[[2]], 4)
  PPV <- round(result_list$conf$byClass[[3]], 4)
  NPV <- round(result_list$conf$byClass[[4]], 4)
...
```{r echo=FALSE}
df_with_metrics <- data.frame(matrix(ncol = 8, nrow = 0))
...
for(tree_num in set_of_tree_nums) {
  df_with_metrics[nrow(df_with_metrics) + 1,] = run_if_with_param(test_data_name, data_df, sub_set_size, tree_num)
}
```
...
```{r}
head(df_with_metrics)
tree_num_in_subset <- factor(df_with_metrics$num_of_trees, level = set_of_tree_nums)

#ggplot(df_with_metrics, aes(x=tree_num_in_subset, y=TNR, color='TNR')) + geom_line(aes(group=2)) + geom_point() + theme_bw()

ggplot(df_with_metrics, aes(tree_num_in_subset)) +
...
#### Test by num_of_tree
#### Test by sub_set_size

```{r eval = FALSE}
# [TODO] Change run_isolation_forest -> run_implemented_isolation_forest
prepare_data_to_test <- prepare_train_data(arrhythmia_data_df)

pred <- factor(run_isolation_forest(prepare_data_to_test, 256, 20, 0.6))
expected <-  factor(prepare_data_to_test$train_label)

run_evaluation_metrics(factor(pred), expected)
