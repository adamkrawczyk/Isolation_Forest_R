...
run_evaluation_metrics <- function(pred, expected) {
  ...
  print(confusionMatrix(pred, expected))
...
run_one_class_svm <- function(split_df) {
  ...
  # training
  model <- svm(x=train_no_label, y=train$label, type='one-classification',kernel='linear')
  pred <- predict(model, test_no_label)
...
pred <- factor(run_one_class_svm(split_train_test_df))
expected <-  factor(split_train_test_df$test[,1])
...
run_random_forest <- function(split_df) {
  ...
  rf_classifier <- randomForest(x=train_no_label, y=train$label, ntree=100, mtry=2, importance=TRUE)
  prediction_for_table <- predict(rf_classifier, test_no_label)
