...
set_of_tree_nums <- list(10, 50, 100, 200)
...
level_order <- factor(df_with_metrics$num_of_trees, level = set_of_tree_nums)

ggplot(df_with_metrics, aes(x=level_order, y=TNR, color='red')) + geom_line(aes(group=2)) + geom_point() + theme_bw()

#ggplot(df_with_metrics, aes(TPR, num_of_trees)) + geom_line() + facet_grid(series ~ .)
```
