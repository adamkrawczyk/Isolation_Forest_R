...
Wykres przedstawia wpływ wartości oznaczająca liczba tworzynych drzew w lasie izolaczynjnym na wyniki działania algorytmu. Jako wyniki podane dane z *confision matrix*.
...
sub_set_size <- 256
data_df <- arrhythmia_data_df
test_data_name <- "arrhytmia"
...
ggplot(df_tree_nums_metrics, aes(tree_num_in_subset)) +
  ...
  labs(
      title = "Wpływ wartości liczba podzbiora danych na parametry z confussion matrix",
      y = "Procent [%]",
      x = "Liczba danych wykorzystana dla tworzenia drzewa"
    )
...
**prepare_sub_set_size_list** - przygotowanie zbióra danych określąjący wielkość zbiorów wykorzystanych dla tworzenia drzewa (10%, 20%, ... ,100% od zbiora danych). Parametry:

* **size_of_data** - rozmiar danych.

Wykres przedstawia wpływ wartości oznaczająca liczba zbiora danych z którego tworzone poszczególne drzewa w lasie izolaczynjnym na wyniki działania algorytmu. Jako wyniki podane dane z *confision matrix*.
...
tree_num <- 20
sub_sets_size_list <- prepare_sub_set_size_list(nrow(data_df))
...
tree_num_in_subset <- factor(df_sub_set_metrics$sub_set_size )

ggplot(df_sub_set_metrics, aes(tree_num_in_subset)) +
  geom_line(aes(y = TPR, color = "TPR/recall", group=2)) +
  geom_line(aes(y = TNR, color = "TNR", group=2)) +
  geom_line(aes(y = PPV, color = "PPV/precision", group=2)) +
  geom_line(aes(y = NPV, color = "NPV", group=2))
  labs(
      title = "Wpływ wartości liczba podzbiora danych na parametry z confussion matrix",
      x = "Procent [%]",
      y = "Liczba danych wykorzystana dla tworzenia drzewa"
    )
...
```{r echo=FALSE, message=FALSE, results='hide'}
# Sprawdzenie wszystkich kombinacyj tree_num i sub_set_size
...
df_show <- df_sub_set_and_tree_num_metrics_ppv[1:10, ]
subset(df_show, select=c("execution_time", "sub_set_size", "num_of_trees", "PPV"))
```
...
df_show <- df_sub_set_and_tree_num_metrics_tpr[1:10, ]
subset(df_show, select=c("execution_time", "sub_set_size", "num_of_trees", "TPR"))
```
